...
def DeletePlayer(player_uid):
    ...
    if not os.path.exists(player_sav_file):
        ...
        for key in ['CommonContainerId', 'DropSlotContainerId', 'EssentialContainerId', 'FoodEquipContainerId', 'PlayerEquipArmorContainerId', 'WeaponLoadOutContainerId']:
    ...
    for item in wsd['CharacterSaveParameterMap']['value']:
        ...
        if item['key']['PlayerUId']['value'] == uuid.UUID(player_uid):
            ...
            remove_items.append(item)
        elif 'SlotID' in player and player['SlotID']['value']['ContainerId']['value']['ID']['value'] in player_container_ids:
            remove_instance_id.append(item['key']['InstanceId']['value'])
            print(
